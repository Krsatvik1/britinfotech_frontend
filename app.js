...
app.get("/", async (req, res) => {
  try {
    ...
    let dataSEOKeywords = await getApiData(
      "home?populate[SEO][populate][0]=Keywords"
    );
    ...
    let dataBlog = await getApiData(
      "blog-component?populate[blogs][populate][0]=Card_Image"
    );
    let dataCompanies = await getApiData(
      "company-heading?populate[companies][populate][0]=Company_Logo"
    );
    ...
    res.render("home", {
      showContactModal: !req.session.contactMalShown,
      dataSEO,
      dataSEOKeywords,
      dataContact,
      dataChooseUs,
...
app.get("/about", async (req, res) => {
  try {
    let dataSEO = await getApiData(
      "about-us?populate[SEO][populate][0]=SEO_image"
    );
    let dataSEOKeywords = await getApiData(
      "about-us?populate[SEO][populate][0]=Keywords"
    );
    let dataContact = await getApiData("contact?populate=*");
    let dataChooseUs = await getApiData("choose-us?populate=*");
    ...
    let dataSocial = await getApiData("social?populate=*");
    let dataEmployees = await getApiData(
      "employees-component?populate[employees][populate][0]=Profile_Pic"
    );
    ...
    res.render("about", {
      showContactModal: !req.session.contactModalShown,
      dataSEO,
      dataSEOKeywords,
      dataContact,
      dataChooseUs,
...
app.get("/services", async (req, res) => {
  try {
    let dataSEO = await getApiData(
      "services-page?populate[SEO][populate][0]=SEO_image"
    );
    let dataSEOKeywords = await getApiData(
      "services-page?populate[SEO][populate][0]=Keywords"
    );
    let dataContact = await getApiData("contact?populate=*");
    let data1 = await getApiData("services-page?populate=*");
    let dataServices = await getApiData(
      "services-page?populate[services][populate][0]=Service_Brief_Image"
    );
    let dataSocial = await getApiData("social?populate=*");
    let dataEmployees = await getApiData(
      "employees-component?populate[employees][populate][0]=Profile_Pic"
...
app.get("/services/:id", async (req, res) => {
  try {
    const { id } = req.params;
    let dataSEO = await getApiData(
      `services?filters[slug]=${id}&populate[SEO][populate][0]=SEO_image`
    );
    let dataSEOKeywords = await getApiData(
      `services?filters[slug]=${id}&populate[SEO][populate][0]=Keywords`
    );
    let dataContact = await getApiData("contact?populate=*");
    ...
    let dataBlog = await getApiData(
      `services?populate[blogs][populate][0]=Card_Image&filters[slug]=${id}`
    );
    let dataSocial = await getApiData("social?populate=*");
...
app.get("/contact", async (req, res) => {
  try {
    ...
    let dataSEOKeywords = await getApiData(
      `contact?populate[SEO][populate][0]=Keywords`
    );
...
app.get("/insights", async (req, res) => {
  try {
    ...
    let dataFeatured = await getApiData(
      `blogs-page?populate[Featured_Article][populate][]=Full_Image`
    );
    let dataBlog = await getApiData(
      "blog-component?populate[blogs][populate][0]=Card_Image"
    );
...
app.get("/insights/:id", async (req, res) => {
  try {
    const { id } = req.params;
    let dataSEO = await getApiData(
      `blogs?filters[slug]=${id}&populate[SEO][populate][0]=SEO_image`
    );
    let dataSEOKeywords = await getApiData(
      `blogs?filters[slug]=${id}&populate[SEO][populate][0]=Keywords`
    );
    let dataContact = await getApiData("contact?populate=*");
    let data1 = await getApiData(`blogs?filters[slug]=${id}&populate=*`);
    let dataContent = await getApiData(
      `blogs?filters[slug]=${id}&populate[Content][populate][0]=Image`
    );
    let dataBlog = await getApiData(
      `blogs?populate[blogs][populate][0]=Card_Image&filters[slug]=${id}`
    );
    let formData = await getApiData(`contact-form-all`);
    let dataSocial = await getApiData("social?populate=*");
    let dataLegal = await getApiData("legal?populate=*");
  ...
  try {
    ...
    if (recaptchaResponse.data.success) {
      const response = await axios.post(
        process.env.STRAPI_URL + "contact-smalls",
        {
          data: {
            email: email,
          },
...
app.post("/contact/submit", async (req, res) => {
  try {
    ...
    if (recaptchaResponse.data.success) {
      const response = await axios.post(
        process.env.STRAPI_URL + "contact-mains",
        {
          data: {
            name: name,
            company: company,
